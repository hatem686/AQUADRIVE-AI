...
class PixhawkFailoverBridge(Node):
    ...
    def comms_callback(
        ...
        if (
            previous_channel == "FIBER_OPTIC"
            and self.current_channel == "WIRELESS_4G_5G"
        ):

            self.get_logger().warn(
                "Fiber Link Broken! "
                "Triggering Pixhawk Hold/Failsafe."
            )

            self.trigger_pixhawk_mode(
                'AUTO.HOLD'
            )

    def trigger_pixhawk_mode(
        self,
        custom_mode: str
    ):

        if (
            not self.pixhawk_connected
            or not self.set_mode_client.wait_for_service(
                timeout_sec=2.0
            )
        ):

            self.get_logger().error(
                "Pixhawk unavailable for mode change!"
            )
